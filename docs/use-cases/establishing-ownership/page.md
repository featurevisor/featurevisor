...
### Ownership by release lane

When you use sets as release lanes, you can assign different owners to each lane. For example, your QA team can have final say on changes promoted from staging to production.

In that case, the `CODEOWNERS` file can be defined as follows:

```{% path="./.github/CODEOWNERS" %}
sets/dev/features/payment_*.yml @payments-team
sets/staging/features/payment_*.yml @payments-team

# leave production lane changes for QA team to handle
sets/production/features/payment_*.yml @qa-team
```
