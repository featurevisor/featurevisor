...
- `datasource` — read/write project entities without touching the filesystem (below).
- `parsed` — yargs-parsed arguments, e.g. `{ foo: 'bar' }` for `--foo=bar`.

## Datasource API

Use `datasource` instead of `fs` + a YAML parser: it honours the project's `parser`, directory overrides, namespace character, and adapter, so a plugin keeps working in a JSON or TOML project without changes.
...
The default `FilesystemAdapter` reads and writes the Git repository. Swapping it points the whole CLI (and every plugin) at a different store — a database, an API, an object store:
...
**Rarely the right answer.** Featurevisor's value is that definitions are reviewed in Git like code — an adapter that moves them into a database gives that up. Before writing one, check whether the actual need is a plugin that *syncs* to the external system while Git stays the source of truth. Say so plainly if the user is about to trade away the GitOps workflow by accident.
