...
| Term                 | Meaning                                                                                  |
| -------------------- | ---------------------------------------------------------------------------------------- |
| **Datafile**         | Static JSON output the SDK loads. One per (environment, tag) and optionally per scope.   |
| **Tag**              | Per-application grouping. Features tag themselves; build emits per-tag datafiles.        |
| **Scope**            | A pre-evaluated datafile produced from a partial known context (e.g. `platform: web`).   |
